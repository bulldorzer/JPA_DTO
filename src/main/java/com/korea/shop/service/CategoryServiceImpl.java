...
public class CategoryServiceImpl implements CategoryService{
    ...
    private final CategiryRepository categiryRepository;
    ...
    public Category addParentCatagory(String name){
        ...
        // 리포지토리에 저장
        categiryRepository.save(category);
        return category;
    };
    ...
    public void checkDuplicateCategory(String name){
        if (categiryRepository.existsByName(name)){
    ...
    public Category addChildCategory(Long parentId,String name){
        ...
        checkDuplicateCategory(name);
        // 부모카테고리 id로 객체 조회
        Category parent = categiryRepository.findById(parentId).orElseThrow(
    ...
    public List<Category> getParentCategories() {
        return categiryRepository.findByParentIdIsNull();
    }

    //  자식 카테고리만 조회
    @Override
    public List<Category> getChildCategories(Long parentId) {
        return categiryRepository.findByParentId(parentId);
    }
